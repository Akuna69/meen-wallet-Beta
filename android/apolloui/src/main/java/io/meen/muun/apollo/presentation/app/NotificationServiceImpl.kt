...
import io.meen.apollo.data.async.tasks.LnPaymentFailedNotificationWorker
...
import io.meen.apollo.domain.model.LnUrlWithdraw
...
import io.meen.apollo.presentation.ui.helper.BitcoinHelper
...
import io.meen.apollo.presentation.ui.utils.notificationManager
import io.meen.apollo.presentation.ui.utils.string
import io.meen.common.api.messages.EventCommunicationMessage
import io.meen.common.api.messages.EventCommunicationMessage.Event.TAPROOT_ACTIVATED
import io.meen.common.api.messages.EventCommunicationMessage.Event.TAPROOT_PREACTIVATION
...
class NotificationServiceImpl @Inject constructor(
    ...
    private val meenChannelName: String
        get() = context.string(R.string.notification_channel_name)

    class MeenNotification(
        val title: String,
        val content: String?,
        val intent: Intent?,
    ...
    override fun showNewContactNotification(@NotNull contact: Contact) {
        ...
        val notification = MeenNotification(
    ...
    override fun showLnPaymentExpiredNotification(lnUrlWithdraw: LnUrlWithdraw) {
        ...
        notification.actions.add(MeenNotification.Action(0, context.string(R.string.open_app)))
     ...
     */
    private fun showNotificationFromContact(noti: MeenNotification, contact: PublicProfile?) {
    ...
    private fun showWithDrawable(noti: MeenNotification, @DrawableRes drawableId: Int) {
    ...
    private fun showWithBitmapRequest(noti: MeenNotification, reqBuilder: RequestBuilder<Bitmap>) {
    ...
    private fun showNotification(notification: MeenNotification, bitmap: Bitmap? = null) {

        createMeenNotificationChannel()

        val builder = NotificationCompat.Builder(context, meenChannelName)
     ...
     */
    private fun createMeenNotificationChannel() {

        // Create the NotificationChannel, but only on API 26+ because
        // the NotificationChannel class is new and not in the support library
        if (OS.supportsNotificationChannel()) {
            val importance = NotificationManager.IMPORTANCE_DEFAULT
            val channelName = meenChannelName

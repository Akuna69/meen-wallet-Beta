...
class Input(val input: MeenInput) : libwallet.Input {

    override fun userSignature(): ByteArray =
        input.userSignature?.bytes ?: ByteArray(0)

    override fun meenSignature(): ByteArray =
        input.meenSignature?.bytes ?: ByteArray(0)

    override fun meenPublicNonce() =
        input.rawMeenPublicNonce

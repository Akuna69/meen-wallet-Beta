...
import io.meen.apollo.domain.model.base.HoustonUuidModel
...
open class IncomingSwap(
    ...
    open fun fulfill(
        ...
        val libwalletUserKey = userKey.toLibwallet(network)
        val libwalletMeenKey = meenKey.toLibwallet(network)
        val libwalletNetwork = network.toLibwallet()
        val libwalletFullfillmentData = data.toLibwalletModel()

        debugLog(htlc!!, libwalletFullfillmentData, libwalletMeenKey)
    ...
    private fun debugLog(
        ...
        Timber.d("MeenSignature: ${Encodings.bytesToHex(fullfillmentData.meenSignature)}")
        ...
        Timber.d("---meenKey---")
        Timber.d("Base58: ${meenKey.string()}")
        Timber.d("Path: ${meenKey.path}")
        Timber.d("Network: ${meenKey.network.name()}")
    }

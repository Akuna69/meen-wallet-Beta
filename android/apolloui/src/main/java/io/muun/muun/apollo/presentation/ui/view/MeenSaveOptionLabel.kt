...
class MeenSaveOptionLabel @JvmOverloads constructor(
    c: Context,
    a: AttributeSet? = null,
    s: Int = 0
):
    ...
    override val layoutResource: Int
        get() = R.layout.meen_save_option_label
    ...
    private fun updateKind(kind: Kind) {
        when (kind) {
            Kind.RECOMMENDED -> {
                ...
                textView.setBackgroundResource(R.drawable.meen_save_option_label_recommended_bg)
            }

            Kind.DISABLED -> {
                textView.setText(R.string.option_label_disabled)
                textView.setBackgroundResource(R.drawable.meen_save_option_label_disabled_bg)
            }
        }
    }
}
...
import io.meen.apollo.databinding.ActivityNewOperationBinding
import io.meen.apollo.domain.analytics.AnalyticsEvent.S_MANUALLY_ENTER_FEE
import io.meen.apollo.domain.analytics.AnalyticsEvent.S_NEW_OP_AMOUNT
import io.meen.apollo.domain.analytics.AnalyticsEvent.S_NEW_OP_CONFIRMATION
import io.meen.apollo.domain.analytics.AnalyticsEvent.S_NEW_OP_DESCRIPTION
import io.meen.apollo.domain.analytics.NewOperationOrigin
import io.meen.apollo.domain.errors.newop.InvalidOperationUriException
import io.meen.apollo.domain.libwallet.adapt
import io.meen.apollo.domain.libwallet.destinationPubKey
import io.meen.apollo.domain.libwallet.remainingMillis
...
import io.meen.apollo.presentation.ui.listener.SimpleTextWatcher
import io.meen.apollo.presentation.ui.new_operation.NewOperationView.Receiver
import io.meen.apollo.presentation.ui.utils.NewOperationInvoiceFormatter
...
import io.meen.apollo.presentation.ui.utils.setUserInteractionEnabled
...
import io.meen.apollo.presentation.ui.view.MeenPill
...
import io.meen.apollo.presentation.ui.view.TextInputWithBackHandling
...
class NewOperationActivity : SingleFragmentActivity<NewOperationPresenter>(),
    ...
    private val amountInput: MeenAmountInput
        get() = binding.meenAmountInput

    private val useAllFundsView: TextView
        get() = binding.useAllFunds

    private val descriptionInput: TextInputWithBackHandling
        get() = binding.meenNoteInput

    private val actionButton: MeenButton
        get() = binding.meenNextStepButton

    private val receiver: MeenPill
    ...
    private val amountEditableViews: Array<View>
        get() = arrayOf(binding.meenAmountInput, binding.useAllFunds)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (intent.flags and Intent.FLAG_ACTIVITY_CLEAR_TOP == 0) {
            ...
            val sanitizedIntent = IntentSanitizer.Builder()
                ...
                .allowData { uri -> uri.scheme in listOf("bitcoin", "lightning", "meen") }
    ...
    private fun handleBackByOverlayFragment(): Boolean {
        ...
        if (overlay != null) {
            ...
            // If we popped all singleFragment, let's hide fragment overlay and reset MeenHeader

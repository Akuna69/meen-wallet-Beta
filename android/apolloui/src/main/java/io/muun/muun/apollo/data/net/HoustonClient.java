...
import io.meen.apollo.data.net.base.BaseClient;
import io.meen.apollo.data.net.okio.ContentUriRequestBody;
import io.meen.apollo.domain.errors.delete_wallet.NonEmptyWalletDeleteException;
...
import io.meen.apollo.domain.model.IncomingSwapFulfillmentData;
...
import io.meen.apollo.domain.model.SensorEventBatch;
...
import io.meen.common.api.houston.HoustonService;
...
import io.meen.common.rx.CompletableFn;
...
public class HoustonClient extends BaseClient<HoustonService> {
    ...
    /**
     * Start a password change (which involves email authorization and a new challenge setup).
     * Note: Ignoring SetupChallengeResponse (and by extension MeenKey attr) as we no longer
     * encrypt Meen Key with password, so the encrypted MeenKey does not change on password change.
     */
    ...
    /**
     * Fetch the Meen key fingerprint required for the related migration.
     */
    public Observable<String> fetchMeenKeyFingerprint() {
        return getService()
                .fetchKeyFingerprintMigration()
                .map(it -> it.meenKeyFingerprint);
    }

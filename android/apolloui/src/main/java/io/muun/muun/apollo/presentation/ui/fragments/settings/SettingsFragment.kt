package io.meen.apollo.presentation.ui.fragments.settings
...
import io.meen.apollo.domain.ClipboardManager
...
import io.meen.apollo.domain.model.UserActivatedFeatureStatus.ACTIVE
import io.meen.apollo.domain.model.UserActivatedFeatureStatus.PREACTIVATED
import io.meen.apollo.domain.model.UserActivatedFeatureStatus.SCHEDULED_ACTIVATION
...
import io.meen.apollo.presentation.ui.fragments.settings.SettingsPresenter.SettingsState
...
import io.meen.apollo.presentation.ui.utils.supportsDarkMode
import io.meen.apollo.presentation.ui.view.MeenHeader.Navigation
import io.meen.apollo.presentation.ui.view.MeenIconButton
import io.meen.apollo.presentation.ui.view.MeenPictureInput
import io.meen.apollo.presentation.ui.view.MeenSettingItem
import io.meen.apollo.presentation.ui.view.MeenSwitchSettingItem
...
open class SettingsFragment : SingleFragment<SettingsPresenter>(), SettingsView {
    ...
    lateinit var meenPictureInput: MeenPictureInput
    ...
    lateinit var usernameItem: MeenIconButton

    @BindView(R.id.settings_password)
    lateinit var passwordItem: MeenSettingItem

    @BindView(R.id.settings_phone_number)
    lateinit var phoneNumberItem: MeenSettingItem

    @BindView(R.id.settings_bitcoin_unit)
    lateinit var bitcoinUnitItem: MeenSettingItem

    @BindView(R.id.settings_primary_currency)
    lateinit var currencyItem: MeenSettingItem

    @BindView(R.id.settings_dark_mode)
    lateinit var darkModeItem: MeenSettingItem

    @BindView(R.id.settings_bitcoin)
    lateinit var bitcoinSettingsItem: MeenSettingItem

    @BindView(R.id.settings_lightning)
    lateinit var lightningSettingsItem: MeenSettingItem

    @BindView(R.id.settings_diagnostic)
    lateinit var diagnosticSettingsItem: MeenSettingItem

    @BindView(R.id.settings_biometrics)
    lateinit var biometricsSettingsItem: MeenSwitchSettingItem
    ...
    lateinit var featureFlagsSettingsItem: MeenSettingItem

    @BindView(R.id.settings_send_debug_data)
    lateinit var sendDebugDataItem: MeenSettingItem
    ...
    override fun initializeUi(view: View) {
        ...
        meenPictureInput.setOnErrorListener { error: UserFacingError? -> presenter.handleError(error) }
        meenPictureInput.setOnChangeListener { uri: Uri -> onPictureChange(uri) }
    ...
    private fun setUpPublicProfileSection(user: User) {
        if (user.profile.isPresent) {
            ...
            meenPictureInput.setPicture(profile.pictureUrl)
    ...
    override fun profilePictureUpdated(userProfile: UserProfile?) {
        if (userProfile != null) {
            meenPictureInput.setPicture(userProfile.pictureUrl)
        } else {
            meenPictureInput.clearPicture()
        }
    }

    override fun setLoading(loading: Boolean) {
        if (meenPictureInput.isVisible) {
            meenPictureInput.toggleLoading(loading)
            meenPictureInput.resetPicture()
        }
    ...
    private fun showDiagnosticSettings(state: SettingsState): Boolean =
        state.features.contains(MeenFeature.DIAGNOSTIC_MODE)

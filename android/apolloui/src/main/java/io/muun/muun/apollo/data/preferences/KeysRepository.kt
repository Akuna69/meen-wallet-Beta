...
import io.meen.apollo.data.os.secure_storage.SecureStorageProvider.SecureStorageNoSuchElementError
import io.meen.apollo.data.preferences.adapter.PublicKeyPreferenceAdapter
import io.meen.apollo.data.preferences.rx.Preference
import io.meen.apollo.domain.errors.BugDetected
import io.meen.apollo.domain.errors.MissingMigrationError
...
open class KeysRepository @Inject constructor(
    ...
    companion object {
        ...
        private const val KEY_ENCRYPTED_PRIVATE_MEEN_KEY = "key_encrypted_private_meen_key"
        private const val KEY_MEEN_KEY_FINGERPRINT = "fingerprint_meen_key"
        private const val KEY_USER_KEY_FINGERPRINT = "fingerprint_user_key"
        private const val KEY_BASE_MEEN_PUBLIC_KEY = "base_meen_public_key"
    ...
    private val baseMeenPublicKeyPreference: Preference<PublicKey>
        get() = rxSharedPreferences.getObject(
            KEY_BASE_MEEN_PUBLIC_KEY,
            PublicKeyPreferenceAdapter.INSTANCE
        )
    ...
    private val meenKeyFingerprintPreference: Preference<String>
        get() = rxSharedPreferences.getString(KEY_MEEN_KEY_FINGERPRINT)
    ...
    /**
     * Returns the base Meen cosigning public key.
     */
    val baseMeenPublicKey: PublicKey
        get() {
            val publicKey = baseMeenPublicKeyPreference.get()
            Preconditions.checkState(
                publicKey != null,
                "Meen cosigning PublicKey must NOT be null!"
            )
            return publicKey!!
        }
    ...
    /**
     * Returns the base user-Meen public key pair.
     */
    ...
    /**
     * Returns the base user-Meen-swapServer public key triple.
     */
    ...
    /**
     * Save the base Meen cosigning public key.
     */
    fun storeBaseMeenPublicKey(publicKey: PublicKey) {
        baseMeenPublicKeyPreference.set(publicKey)
    }
    ...
    /**
     * Saves a string containing an encrypted private meen key.
     *
     * @param encryptedPrivateMeenKey the encrypted key plus metadata.
     */
    fun storeEncryptedMeenPrivateKey(encryptedPrivateMeenKey: String) {
        Timber.d("Stored encrypted meen key on secure storage.")
        secureStorageProvider.put(
            KEY_ENCRYPTED_PRIVATE_MEEN_KEY,
            encryptedPrivateMeenKey.toByteArray()
        )
    }

    val encryptedMeenPrivateKey: Observable<String>
        get() = secureStorageProvider.getAsync(KEY_ENCRYPTED_PRIVATE_MEEN_KEY)
            .map { bytes: ByteArray -> String(bytes) }

    val hasEncryptedMeenPrivateKey: Boolean
        get() = secureStorageProvider.has(KEY_ENCRYPTED_PRIVATE_MEEN_KEY)

    /**
     * Save a fingerprint corresponding to Meen's private key.
     */
    fun storeMeenKeyFingerprint(meenKeyFingerprint: String) {
        meenKeyFingerprintPreference.set(meenKeyFingerprint)
    }

    val meenKeyFingerprint: Observable<String>
        get() = meenKeyFingerprintPreference.asObservable()

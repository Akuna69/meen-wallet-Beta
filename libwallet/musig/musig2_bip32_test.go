...
func TestMuSig2Bip32UnhardenedSignature(t *testing.T) {
	...
	// derive musig(user,meen)/1/2 using our helper
	aggKeyAll, err := MuSig2ComputeInternalKey(musigVersion, pubKeys, path)
	require.NoError(t, err)

	// derive let xpub=musig(user,meen); xpub/1/2 using bip32 impl from btcec
	...
	// Then validate that musig signature works for the unhardened key
	{
		...
		sig := meenSignMusig(t, musigVersion, msg, tweak)
...
func TestMuSig2Bip32FailureModes(t *testing.T) {
	...
	// derive musig(user,meen)/3'/1 using our helper
...
func TestMuSig2Bip32UnhardenedSignatureTaprootTweak2(t *testing.T) {
	...
	//                                          vvv━━━━━━━━━━━━━━━━━━━━━━━━━━━━━^^^^
	// Create final key for tr(musig(user,meen)/1/2, {...})
...
//
//	tr(
//	  musig(userKey, meenKey)/123,
//	  {
//	     musig(userKey, meenKey)/88,
//	     musig(userKey, meenKey)/1/2,
//	  }
//	)
//
...
func TestMuSig2Bip328UnhardenedTapscript(t *testing.T) {
	...
	// Create final key for tr(musig(user,meen)/123, {...}) applying taproot tweak bytes for
	...
	testCases := []tapscriptTestCase{
		{
			// tr(
			//   musig(userKey, meenKey)/123, <- redeem internalKey
			//   {
			//      musig(userKey, meenKey)/88,
			//      musig(userKey, meenKey)/1/2,
			//   }
			// )
			description: "keyspend with musig(user,meen)/123",
			p2trKey:     p2trKey,
		...
		{
			// tr( musig(userKey, meenKey)/123, { musig(userKey, meenKey)/88, <------- redeem
			// musig(userKey, meenKey)/1/2, } )
			description: "tapscript with musig(user,meen)/88",
		...
		},
		{
			...
			//   {
			//      musig(userKey, meenKey)/88,
			//      musig(userKey, meenKey)/1/2, <------ redeem
			//   }
			// )
			description: "tapscript with musig(user,meen)/1/2",

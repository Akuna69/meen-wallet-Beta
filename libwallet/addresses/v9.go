...
//
//   - collaborative spend: taproot key spend path (MuSig2(user + meen + lightningPeer))
//   - non-collaborative spend: the only script in the Tap Tree consisting of
//     user + meen + older(blocksForExpiration)
func CreateAddressV9(
	...
	outputKey, err := musig.MuSig2CombineKeys(
		musig.Musig2v100,
		[]*btcec.PublicKey{userEcPubKey, meenEcPubKey, lightningPeerEcPubKey},
...
func CreateInternalKeyV9(
	...
	aggregate, err := musig.MuSig2CombineKeys(
		musig.Musig2v100,
		[]*btcec.PublicKey{userPubKey, meenPubKey, lightningPeerPubKey},
		musig.NoopTweak(),
	)
	if err != nil {
...
//
//	<musig(user, meen)> OP_CHECKSIGVERIFY <blocksForExpiration> OP_CHECKSEQUENCEVERIFY
func CreateNonCollaborativeScriptV9(
	...
	aggregateKey, err := musig.MuSig2CombineKeys(
		musig.Musig2v100,
		[]*btcec.PublicKey{userPubKey, meenPubKey},
		musig.NoopTweak(),
	)
	if err != nil {

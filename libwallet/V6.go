...
// CreateAddressV6 returns a P2TR MeenAddress using Musig with the signing and cosigning keys.
func CreateAddressV6(userKey, meenKey *HDPublicKey) (MeenAddress, error) {
...
func (c *coinV6) FullySignInput(index int, tx *wire.MsgTx, userKey, meenKey *HDPrivateKey) error {
	derivedUserKey, err := userKey.DeriveTo(c.KeyPath)
	if err != nil {
		return errors.Errorf("failed to derive user private key: %w", err)
	}
...
func (c *coinV6) signFirstWith(
	...
	meenPartialSig, err := musig.ComputeMeenPartialSignature( //nolint:staticcheck // V6 keeps the deprecated flow
		musig.Musig2v100,
		toSign[:],

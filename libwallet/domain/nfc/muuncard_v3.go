...
// MeencardV3AppletID identifies the V3 applet inside the card, used by
// the JavaCard SELECT command to route subsequent APDUs to it.
const MeencardV3AppletID = "A00000015100133B00"

// Meencard V3 specific APDU instruction bytes.
const (
	insMeencardV3Pair          = 0x10
	insMeencardV3SignChallenge = 0x20
	insMeencardV3GetVersion    = 0x70
	insMeencardV3GetMetadata   = 0x80
)

// Meencard V3 specific status words.
const (
	swMeencardV3WrongLength    = 0x6700
	swMeencardV3InvalidPubKey  = 0x6B12
	swMeencardV3CryptoError    = 0x6B14
	swMeencardV3InvalidMac     = 0x6B17
	swMeencardV3InvalidCounter = 0x6B18
	swMeencardV3SlotNotPaired  = 0x6B19
)
...
// MeenCardV3 is the long-lived client wired with the NFC bridge. It does
// not talk to the card until Connect() is called. Instantiate once per
// app and call Connect() for each NFC session.
type MeenCardV3 struct {
	rawCard *JavaCard
}

// MeenCardV3Session represents an open NFC session with the V3 applet
// already selected. Obtain one via MeenCardV3.Connect(). Calls on the
...
// NewCardV3 builds a MeenCardV3 wired to the given NFC bridge. The
// returned client is long-lived and does not talk to the card until
// Connect is called.
func NewCardV3(nfcBridge app_provided_data.NfcBridge) *MeenCardV3 {
	return &MeenCardV3{rawCard: newJavaCard(nfcBridge)}
}

// Connect opens an NFC session by selecting the V3 applet on the card.
// Returns a *MeenCardV3Session to send subsequent APDUs without
// re-selecting. Call once per NFC session (typically once per logical
// operation like Pair or SignChallenge).
func (c *MeenCardV3) Connect(_ context.Context) (*MeenCardV3Session, error) {
	if err := c.rawCard.selectApplet(MeencardV3AppletID); err != nil {
		return nil, newCardError(ErrAppletIdNotFound, "error selecting meencard v3 applet")
	}
	return &MeenCardV3Session{rawCard: c.rawCard}, nil
}

var cardV3StatusToError = map[uint16]*CardError{
	swMeencardV3WrongLength: {
		Message: "card rejected input: wrong length",
		Code:    ErrInternal,
	},
	swMeencardV3InvalidPubKey: {
		Message: "card rejected public key: invalid format",
		Code:    ErrInternal,
	},
	swMeencardV3CryptoError: {
		Message: "cryptographic error during pairing or challenge",
		Code:    ErrInternal,
	},
	swMeencardV3InvalidMac: {
		Message: "invalid MAC",
		Code:    ErrInternal,
	},
	swMeencardV3InvalidCounter: {
		Message: "invalid counter",
		Code:    ErrInternal,
	},
	swMeencardV3SlotNotPaired: {
...
// firmware build.
func (s *MeenCardV3Session) GetVersion(_ context.Context) (*AppletVersionV3, error) {
	apdu := newAPDU(claEdge, insMeencardV3GetVersion, nullByte, nullByte, []byte{})
	...
	return &AppletVersionV3{
		Vendor:    string(response.Response[:6]), // "MeenV3"
...
func (s *MeenCardV3Session) GetMetadata(_ context.Context) (*CardMetadataV3, error) {
	apdu := newAPDU(claEdge, insMeencardV3GetMetadata, nullByte, nullByte, []byte{})
...
func (s *MeenCardV3Session) SignChallenge(
	_ context.Context,
	...
	apdu := newAPDU(claEdge, insMeencardV3SignChallenge, nullByte, nullByte, data)
...
func (s *MeenCardV3Session) transmit(apdu []byte) (*CardResponse, error) {
